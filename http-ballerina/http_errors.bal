...
// # Defines the common error type for the module
// public type Error ListenerError|ClientError;

public class HTTPError {
    private isolated function createError(string typeName, string errorMessage, error cause) returns error {
        match typeName {
            "GenericClientError" => {
                return error GenericClientError(errorMessage, cause);
            }
            "HeaderNotFoundError" => {
                return error HeaderNotFoundError(errorMessage, cause);
            }
            "GenericListenerError" => {
                return error GenericListenerError(errorMessage, cause);
            }
            "IdleTimeoutError" => {
                return error IdleTimeoutError(errorMessage, cause);
            }
            "SslError" => {
                return error SslError(errorMessage, cause);
            }            
            _ => {
                io:println("-------------------------------");
                io:println(typeName);
